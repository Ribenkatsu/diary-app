...
PRIVACY_LAST_UPDATED = "2026年9月19日"
TERMS_LAST_UPDATED = "2026年9月19日"
PRIVACY_OPERATOR = "日記アプリ運営者"
# 公開前に、利用者から問い合わせを受けられる公開用メールアドレスへ変更してください。
PRIVACY_CONTACT_EMAIL = "afiyong50@gmail.com"
...
def render_privacy_policy(show_heading=True):
    if show_heading:
        st.markdown("## 🔒 プライバシーポリシー")
    st.caption(f"最終更新日：{PRIVACY_LAST_UPDATED}")

    st.markdown(
        f"""
「日記アプリ」（以下「本サービス」といいます）は、利用者の情報を次の方針に基づいて取り扱います。

### 1. 取得する情報
本サービスでは、サービス提供のために次の情報を取得・保存する場合があります。

- メールアドレス
- アカウント認証に必要な情報
- 利用者が入力した日記の内容
- アプリ内で設定した表示色、表示設定等
- 本サービスの利用に伴ってシステム上生成される情報

パスワードは、本サービス運営者が平文で保存・閲覧するものではなく、認証サービスであるSupabaseを利用して管理します。
...
取得した情報を広告目的で販売することはありません。

### 3. 日記内容について
日記には個人的な情報が含まれる可能性があります。

現在のβ版では、健康・医療情報、金融情報、生体情報その他の特に機微な情報の入力は避けてください。

日記データはログインしたユーザーごとに分けて保存し、他のユーザーの日記を閲覧できないようアクセス制御を行っています。

### 4. 外部サービスの利用
本サービスでは、運営に必要な範囲で次の外部サービスを利用しています。

- **Supabase**：ユーザー認証、日記データおよび設定データの保存
- **Streamlit Community Cloud / Snowflake**：Webアプリの公開・実行
- **Google Gmail SMTP**：登録確認メール等の認証メール送信

これらのサービスを利用するため、メールアドレス、通信情報その他必要な情報が各サービス提供者によって処理される場合があります。

### 5. 第三者提供
法令に基づく場合その他正当な理由がある場合を除き、利用者の個人情報を本人の同意なく第三者へ販売または提供しません。

ただし、本サービスの提供に必要なクラウドサービス、認証サービス、メール送信サービスへの取扱いの委託は除きます。

### 6. 情報の保護
本サービスでは、個人情報および日記データの漏えい、紛失、不正アクセス等を防止するため、ユーザー認証、ユーザーごとのデータアクセス制御、HTTPSによる通信等の合理的な安全管理措置を講じます。

ただし、インターネット上のサービスについて完全な安全性を保証するものではありません。

### 7. データの確認・訂正・削除
利用者は、自身の登録情報や保存した日記について、確認、訂正または削除を求めることができます。

アカウントおよび関連データの削除を希望する場合は、下記のお問い合わせ先までご連絡ください。

### 8. Cookie等について
本サービスおよび利用する外部サービスでは、ログイン状態の維持、セキュリティ確保、サービス提供等のためにCookieまたはこれに類する仕組みが利用される場合があります。

### 9. β版について
本サービスは現在β版です。機能、データ保存方法、利用する外部サービス等は今後変更される場合があります。重要な変更を行った場合は、本プライバシーポリシーを更新します。

### 10. プライバシーポリシーの変更
本ポリシーは、法令、本サービスの内容または利用する外部サービスの変更等に応じて改定することがあります。変更後の内容は、本サービス上に掲載した時点から適用します。

### 11. 運営者・お問い合わせ
**運営者：{PRIVACY_OPERATOR}**  
**お問い合わせ先：{PRIVACY_CONTACT_EMAIL}**

個人情報の確認、訂正、削除、利用停止その他のお問い合わせは、上記窓口までお願いいたします。
        """
    )
...
def render_terms_of_service(show_heading=True):
    if show_heading:
        st.markdown("## 📜 利用規約")
    st.caption(f"最終更新日：{TERMS_LAST_UPDATED}")

    st.markdown(
        f"""
この利用規約（以下「本規約」といいます）は、「日記アプリ」（以下「本サービス」といいます）の利用条件を定めるものです。

本サービスを利用する方（以下「利用者」といいます）は、本規約および別途定めるプライバシーポリシーに同意したうえで、本サービスをご利用ください。

### 第1条　本サービスについて
本サービスは、利用者が日々の出来事等を記録し、保存・閲覧するための日記サービスです。

本サービスは現在**β版**として提供しており、機能、仕様、デザイン、保存方法等を変更する場合があります。

現在のβ版は、個人による評価・テストを目的として無料で提供しています。

### 第2条　アカウント登録
本サービスの利用には、メールアドレスおよびパスワードによるアカウント登録が必要です。

利用者は、自身のアカウント情報を適切に管理し、第三者に不正に利用されないよう注意してください。

第三者による不正利用が疑われる場合は、速やかにお問い合わせ先までご連絡ください。

### 第3条　日記データ
利用者が本サービスに入力した日記その他の文章について、その権利は原則として利用者に帰属します。

本サービスは、サービス提供に必要な範囲で当該データを保存、表示、処理します。

日記データはユーザーごとに分けて管理し、他の利用者から閲覧できないようアクセス制御を行います。

### 第4条　β版で入力してはいけない情報
現在のβ版では、次のような特に機微な情報を入力しないでください。
...
β版の公開環境や外部サービスの利用条件に合わせ、安全に運用するための制限です。
...
### 第6条　外部サービス
本サービスでは、サービス提供のため次の外部サービスを利用しています。

- **Supabase**：ユーザー認証、日記・設定データの保存
- **Streamlit Community Cloud / Snowflake**：Webアプリの公開・実行
- **Google Gmail SMTP**：アカウント確認等のメール送信

各外部サービスの障害、仕様変更、サービス終了等により、本サービスの全部または一部が利用できなくなる場合があります。

### 第7条　サービスの変更・停止
運営者は、保守、障害対応、セキュリティ上の必要性、利用する外部サービスの変更その他合理的な理由がある場合、本サービスの内容を変更、一時停止または終了することがあります。

β版では、予告なくメンテナンスや機能変更を行う場合があります。

### 第8条　データの保存について
運営者はデータの適切な保存に努めますが、β版であることや外部サービスを利用していることから、サービス停止、障害その他の事情によりデータを利用できなくなる可能性があります。

特に重要な内容については、利用者自身でも別途記録を保管することをおすすめします。

### 第9条　保証および責任
運営者は、本サービスが常に正常に動作すること、すべての環境で利用できること、データが永続的に保存されること等を保証するものではありません。

本サービスに関連して利用者に損害が生じた場合の責任については、適用される法令に従います。

運営者の故意または重大な過失による場合その他法令上免責が認められない場合を除き、間接的・付随的な損害等については、法令上認められる範囲で責任を負わないものとします。

### 第10条　アカウントの停止・削除
利用者が本規約に違反した場合、不正利用が確認された場合、または本サービスの安全な運営に重大な支障がある場合、運営者は必要な範囲でアカウントの利用を制限または停止することがあります。

利用者が自身のアカウントおよび関連データの削除を希望する場合は、お問い合わせ先までご連絡ください。

### 第11条　プライバシー
個人情報その他の利用者情報の取扱いについては、別途掲載する**プライバシーポリシー**に従います。

### 第12条　本規約の変更
運営者は、本サービスの内容変更、法令の変更その他必要に応じて本規約を改定することがあります。

重要な変更を行う場合は、本サービス上で分かりやすい方法によりお知らせします。

### 第13条　準拠法
本規約および本サービスには、日本法を適用します。

本サービスに関して問題が生じた場合は、利用者と運営者との間で誠実に協議し、解決を図るものとします。
...
def render_beta_sensitive_info_notice():
    st.caption(
        "⚠️ β版では、健康・医療、銀行口座・クレジットカード等の金融情報、"
        "生体情報など、特に機微な情報は入力しないでください。"
    )
...
def render_auth_page(client):
    ...
    login_tab, signup_tab, terms_tab, privacy_tab = st.tabs([
        "ログイン",
        "新規登録",
        "利用規約",
        "プライバシーポリシー",
    ...
    with signup_tab:
        with st.form("signup_form"):
            ...
            signup_agreed = st.checkbox(
                "利用規約とプライバシーポリシーに同意します",
                key="signup_agreed",
            )

            st.caption(
                "内容は上の「利用規約」「プライバシーポリシー」タブから確認できます。"
            )
        ...
        if signup_submitted:
            ...
            elif not signup_agreed:
                st.warning("新規登録には、利用規約とプライバシーポリシーへの同意が必要です。")
...
if page == "✏️ 今日の日記":
    ...
    render_beta_sensitive_info_notice()
...
# =========================================
    ...
    render_page_hero("📜", "利用規約")
    render_terms_of_service(show_heading=False)

elif page == "🔒 プライバシーポリシー":
    render_page_hero("🔒", "プライバシーポリシー")
    render_privacy_policy(show_heading=False)
